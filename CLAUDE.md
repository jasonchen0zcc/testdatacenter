...
- **测试驱动开发（TDD）**：所有新功能、Bug 修复、重构必须遵循 TDD 规范——先写失败的测试，再写最小实现使测试通过，最后重构。测试覆盖率目标 ≥ 80%。
- **设计文档用中文**：所有规格说明书（`docs/superpowers/specs/`）和计划文档（`docs/superpowers/plans/`）统一使用中文编写。
- **代码审查**：每个 Task 完成后必须经过 Spec Review 和 Code Quality Review 两轮审查。
- **独立工作树**：功能开发必须在 Git Worktree 中进行，避免污染主分支。
...
4. **subagnet-driven-development** - Execute plan tasks
   - Per task: 写失败测试 → 运行确认失败 → 最小实现 → 运行确认通过 → 自审 → Spec review → Quality review
